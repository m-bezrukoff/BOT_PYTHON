...
class Indicators:
    def __init__(self, data):
        ...
        self.np_open = {frame: np.array([i['open'] for i in self.charts[frame]]) for frame in self.params['time_frames']}
        self.np_close = {frame: np.array([i['close'] for i in self.charts[frame]]) for frame in self.params['time_frames']}
        self.np_high = {frame: np.array([i['high'] for i in self.charts[frame]]) for frame in self.params['time_frames']}
        self.np_low = {frame: np.array([i['low'] for i in self.charts[frame]]) for frame in self.params['time_frames']}
    ...
    def complete_indicators_update(self):
        for frame in self.params['time_frames']:
            t1 = time()
            row, points = self.indicator_functions(frame, full_range=True)
            for key in row[frame].keys():
    ...
    def indicator_functions(self, frame, full_range=False):
        ...
        row[frame]['ema_s'] = tb.EMA(self.np_close[frame][0 if full_range else -300:], 100)
        ...
        # row[frame]['ema_m'] = JMA(
        #     self.np_close[frame][0 if full_range else -self.params['ema_m'] * 4:],
        #     period=self.params['ema_m'], phase=self.params['ema_m_phase'], power=self.params['ema_m_power'])

        # row[frame]['ema_f'] = JMA(
        #     self.np_close[frame][0 if full_range else -self.params['ema_f'] * 4:],
