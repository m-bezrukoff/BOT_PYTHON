...
def volume_prediction():
    ...
    if pickle_file is None:
        df = pandas.DataFrame(list(
            __poloniex_chart_data__.find({}, {'date': 1, 'volume': 1, 'quoteVolume': 1, 'market': 1}).sort(
                [('date', pymongo.ASCENDING), ('market', pymongo.ASCENDING)])))
        ...
        for market in markets:
            ...
            market_df['prediction_volume_percentage'] = (market_df['volume'].shift(-n_ticks_in_future) - market_df[
                'volume']) / market_df['volume']
            market_df['prediction_volume_percentage'].replace([numpy.inf, -numpy.inf], numpy.NaN, inplace=True)
            buy_percentage = 0.4
            sell_percentage = -0.2
            market_df['prediction'] = [[[0, -1][chng_perc < sell_percentage], 1][chng_perc > buy_percentage] for
                                       chng_perc in market_df['prediction_volume_percentage']]
...
def price_regression():
    ...
    filtered_data = numpy.array([[[0, 1][d['type'] == 'buy'], d['rate'], d['amount'], d['total'],
                                  float(time.mktime(time.strptime(d['date'], '%Y-%m-%d %H:%M:%S'))), d['rate'],
                                  datetime.strptime(d['date'], '%Y-%m-%d %H:%M:%S')] for d in data])
    ...
    if pickle_file is None:
        ...
        x_train, x_test, y_train, y_test = cross_validation.train_test_split(train_data[:, :-2], train_data[:, -2],
                                                                             test_size=0.05)
    ...
    plot_results(filtered_data[start_test_index:-10, -2], forecast_test, forecast_set,
                 filtered_data[start_test_index:, -1])
...
def zrx_regression():
    ...
    try:
        ...
        pickle_file = None
    if pickle_file is None:
        df = pandas.DataFrame(list(__poloniex_zrx_trade_history__.find({},
                                                                       {'date': 1, 'amount': 1, 'rate': 1, 'total': 1,
                                                                        'type': 1, 'market': 1})))
        df.drop(['_id'], 1, inplace=True)
